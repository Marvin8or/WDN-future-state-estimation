...
class ConfigGenerator:
    ...
    def __init__(self, inp_file, demand_is_quantile, diff_demand_is_quantile):
        """
        ...
        self.demand_is_quantile = demand_is_quantile
        self.diff_demand_is_quantile = diff_demand_is_quantile
        # Extract configuration information for each network component
        self.junctions_dict = defaultdict(dict)
    ...
    def junction_config(self):
        ...
        for junction_id in self.wn.junction_name_list:
            ...
            if self.demand_is_quantile:
                self.junctions_dict[junction_id][
                    "junc_demand_hi"
                ] = np.percentile(
                    junction.base_demand * demand_pattern.multipliers, 75
                )
                self.junctions_dict[junction_id][
                    "junc_demand_lo"
                ] = np.percentile(
                    junction.base_demand * demand_pattern.multipliers, 25
                )
            else:
            ...
            if self.diff_demand_is_quantile:
                self.junctions_dict[junction_id][
                    "junc_demand_diff_hi"
                ] = np.percentile(
